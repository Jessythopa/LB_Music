...
from SlayerX.utils.jarvis_ban import admin_filter
...
import asyncio
import time
from SlayerX.utils.database import add_served_chat
from pyrogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton
from SlayerX.utils.database import get_assistant
import asyncio
from SlayerX.misc import SUDOERS
from SlayerX.mongo.afkdb import PROCESS
from pyrogram import Client, filters
...
from SlayerX.utils.database import get_assistant, is_active_chat
...
def circle(pfp, size=(500, 500), brightness_factor=10):
    pfp = pfp.resize(size, Image).convert("RGBA")
    pfp = ImageEnhance.Brightness(pfp).enhance(brightness_factor)
    bigsize = (pfp.size[0] * 3, pfp.size[1] * 3)
    mask = Image.new("L", bigsize, 0)
    draw = ImageDraw.Draw(mask)
    draw.ellipse((0, 0) + bigsize, fill=255)
    mask = mask.resize(pfp.size, Image)
    mask = ImageChops.darker(mask, pfp.split()[-1])
    pfp.putalpha(mask)
    return pfp

def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("SlayerX/assets/wel2.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp, brightness_factor=brightness_factor) 
    pfp = pfp.resize((635, 635))
    draw = ImageDraw.Draw(background)
    font = ImageFont.truetype('SlayerX/assets/font.ttf', size=70)
    welcome_font = ImageFont.truetype('SlayerX/assets/font.ttf', size=61)
